...
#v 0.9.4p4 | 05-15 | github.com/dyndna
#@sbamin
...
if [ "$1" == "-h" ] || [ "$1" == "--h" ] || [ "$#" -lt 3 ]; then
	...
	MD5CGHUB="$3"
	MYKEY="./info/cgkey"
	MYBAMDONEDIR="./processed/${MYBAMID}"
	...
	if [ -z "$MYWORKDIR" ]; then
		...
		if [ $(pwd) != "$MYWORKDIR" ]; then
			...
			mkdir -p {ingest,logs,out,processed}
			ls -alh
			echo "######################################################"
			echo -e "\n## SANITY CHECK ##\n"
			...
			if [ -d "$MYBAMDONEDIR" ]; then
				echo -e "Sample looks already processed, please check htseq output at ./out/${MYOUTID} , if any. Delete or rename "$MYBAMDONEDIR" to proceed.\n#### Exiting with ERROR ####\n" >&2
				exit 1
			else
				...
				if [ "$MD5BAM" != "$MD5CGHUB" ]; then
					...
					if [ ! -f "$MYSORTEDBAM" ]; then
						echo -e "\n#### START_NAME_SORTING_BAM_DATESTAMP: $(date) ####"
					...
					if [ ! -f "$MYSORTEDBAM" ]; then
						...
						echo -e "\n### END_HTSeq_DATESTAMP: $(date) ###\n"

						echo "######################################################"
						echo -e "\n## Moving bams to processed directory ##\n"

						mv "./ingest/${MYBAMID}" ./processed/ &
						mv "./ingest/${MYBAMID}.gto" ./processed/ &

						sleep 5
						MYTOTAL=$(ls ./processed/*.gto | wc -l)

						echo -e "\n### END_DATESTAMP: $(date) ###\n"
						echo -e "################## Processed "$MYTOTAL" bams ##################\n"
					fi
				fi
			fi
		fi
